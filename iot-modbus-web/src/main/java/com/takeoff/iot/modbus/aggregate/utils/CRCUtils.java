package com.takeoff.iot.modbus.aggregate.utils;
...
public class CRCUtils {
        ...
        public static void main(String args[]) throws UnsupportedEncodingException {
            String d = "!AIVDM,1,1,,B,B6:k>eP09R1akNT?@;V39`v021Mk,0*6A";
            String[] forcrc = d.split(",");
            String crc  = forcrc[forcrc.length-1].split("\\*")[1];
            String jydata = "";
            for(int i=0;i<forcrc.length-1;i++){
                jydata += forcrc[i];
            }
            jydata = jydata.substring(1,jydata.length()-1) +","+ forcrc[forcrc.length-1].split("\\*")[0];
            byte[] rb = getCrc16(jydata.getBytes());
            String r = new String(rb);
            byte[] rr = "6A".getBytes();
